...
export function exportImageAsBlob(artCanvas, gridData, config, triHeight, W_half, imageRegistry) {
    const tempCanvas = document.createElement("canvas");
    tempCanvas.width = artCanvas.width;
    tempCanvas.height = artCanvas.height;
    const tCtx = tempCanvas.getContext("2d");
...
export function exportSVGAsString(artCanvas, gridData, config, triHeight, W_half) {
    const w = artCanvas.width;
    const h = artCanvas.height;
    let svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">`;
