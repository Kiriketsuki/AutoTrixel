...
### Must-Have
- **Create canvas**: Initialize a new canvas with configurable rows, columns, and triangle size. Outputs a `.trkixel.json` state file. Acceptance: running `trkixel create --rows 10 --cols 10 -o canvas.trkixel.json` produces a valid state file.
- **Paint cells**: Set one or more cells to a color by grid coordinate. Acceptance: `trkixel paint -i canvas.trkixel.json --cell 3,4 --color "oklch(60% 0.15 200)"` updates the cell in the state file.
- **Flood fill**: Fill a contiguous region from a seed cell. Acceptance: `trkixel fill -i canvas.trkixel.json --cell 3,4 --color "#ff0000"` fills all adjacent same-colored cells.
- **Export PNG**: Render the canvas to a PNG file using node-canvas. Acceptance: `trkixel export -i canvas.trkixel.json -o art.png --format png` produces a valid PNG matching the grid contents.
- **Export SVG**: Render the canvas to an SVG file. Acceptance: `trkixel export -i canvas.trkixel.json -o art.svg --format svg` produces a valid SVG with correct triangle geometry.
- **Load palette**: Import a `.gpl` palette file into the canvas state. Acceptance: `trkixel palette -i canvas.trkixel.json --import palette.gpl` stores the palette colors in the state file.
- **Batch run**: Execute a JSON array of operations in sequence. Acceptance: `trkixel run instructions.json` applies each operation and writes the final state.

### Should-Have
- **Subdivide**: Split a trixel into 4 sub-triangles. `trkixel subdivide -i canvas.trkixel.json --cell 3,4`
...
| File | Status | Notes |
|:-----|:-------|:------|
| `src/logic/trkixel/geometry.js` | Reuse as-is | Pure JS, no DOM. Triangle math, hit-testing, barycentric coords. |
| `src/logic/trkixel/utils.js` | Reuse as-is | OKLCH/RGB/Hex color conversion. Pure math. |
| `src/logic/palette-utils.js` | Reuse as-is | GPL format parser/generator. Pure JS. |
| `src/logic/trkixel/actions.js` | Extract | Paint, fill, stroke ops. Currently coupled to closure state -- needs parameter injection. |
| `src/logic/trkixel/drawing.js` | Adapt | Canvas rendering. Needs canvas adapter interface (browser vs node-canvas). |
| `src/logic/trkixel/export.js` | Adapt | PNG: swap `document.createElement("canvas")` for adapter. SVG: return string instead of triggering download. |
| `src/logic/createTrKixel.js` | Decompose | 1,167-line closure. Extract state management + operations into a headless `Engine` class/factory. Browser app rewires to use the same core. |
...
cli/
  ...
  commands/
    create.js            -- `trkixel create`
    paint.js             -- `trkixel paint`
    fill.js              -- `trkixel fill`
    export.js            -- `trkixel export`
    palette.js           -- `trkixel palette`
    run.js               -- `trkixel run` (JSON batch)
```

**Canvas State Format** (`.trkixel.json`):
...
Feature: trkixel-cli
  ...
  Rule: Canvas creation
    ...
    Scenario: Create fails when output path is not writable
      ...
      When I run `trkixel create --rows 10 --cols 10 -o /readonly/canvas.trkixel.json`
  ...
  Rule: Batch execution
    ...
    Scenario: Batch fails on invalid operation
      ...
      When I run `trkixel run instructions.json`
      Then the CLI exits with code 1
...
| T4   | CLI scaffold: `cli/index.js` with commander, `bin` field in package.json, `trkixel --help` | Med | T1 | pending |
...
| T7   | Wire browser `createTrKixel.js` to use `src/core/engine.js` -- verify no regression | High | T1, T2.1 | pending |
...
- [ ] `trkixel run` successfully executes a multi-step instruction file
- [ ] Canvas state `.trkixel.json` round-trips correctly (create -> paint -> export -> re-import)
...
- Geometry module: `src/logic/trkixel/geometry.js`
